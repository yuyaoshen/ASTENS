...
public class codeNormalizerJava {
				...
				if (language.equals("java")) {
					...
					if (codeNormalization) {
						...
						for (int j = 0; j < lines.length; j++) {
							...
							while (m.find()) {
								name = m.group();
								if (!keywordsList.mainSet.contains(name) && !keywordsList.keywordsJava.contains(name)
							...
							while (m.find()) {
								name = m.group();
								if (!keywordsList.mainArgs.contains(name) && !keywordsList.keywordsJava.contains(name)
